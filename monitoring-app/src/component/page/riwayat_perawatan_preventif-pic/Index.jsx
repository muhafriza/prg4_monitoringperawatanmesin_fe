...
export default function RiwayatPreventifPIC({ onChangePage }) {
  ...
  const exportToExcel = async (periodFilter = null) => {
    try {
      ...
      columnWidthsPreventif.forEach(
        (w, i) => (worksheetPreventif.getColumn(i + 1).width = w)
      );
      ...
      if (fetchDataDetailSP && fetchDataDetailSP.length > 0) {
        ...
        columnWidthsSparepart.forEach(
          (w, i) => (worksheetSparepart.getColumn(i + 1).width = w)
        );
  ...
  const exportToPDF = async (periodFilter = null) => {
    try {
      ...
      const addHeader = (
        yPosition,
        title = "LAPORAN DATA PERAWATAN PREVENTIF"
      ) => {
        try {
  ...
  const fetchDetailSP = async () => {
    try {
      const data = await UseFetch(
        API_LINK +
          "TransaksiPreventif/getdetailSparepartPerawatanPreventifSelesaiPIC",
        {
          p1: "Selesai",
          p2: upt,
        }
